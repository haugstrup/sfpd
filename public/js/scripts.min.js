...
angular.module('sfpdApp').run(['$templateCache', function($templateCache) {
  ...
  $templateCache.put('public/views/player.html',
    "<div class=player-info><h2>{{player.display_name}}</h2><div class=section ng-if=group><h3>Last result: {{group.heat.season.name}}, {{group.heat.name}}</h3><table class=\"table table-scores table-condensed\"><thead><tr><th>Player</th><th ng-repeat=\"game in group.games\" class=text-center title={{game.machine.name}}>{{game.machine.shortname}}</th><th class=text-center><b>Total</b></th></tr></thead><tbody><tr ng-repeat=\"player in group.players\" class={{highlightRow(player)}}><td>{{player.display_name}}</td><td ng-repeat=\"game in group.games\" class=text-center>{{ pointsForPlayer(game.results, player.player_id) }}pt.</td><td class=text-center><b>{{ pointsForPlayer(group.points, player.player_id) }}pt.</b></td></tr></tbody></table></div><div class=section ng-if=\"machines.length > 0\"><h3>Machine choices</h3><table class=\"table table-striped\"><tbody><tr ng-repeat=\"machine in machines\"><td>{{machine.name}}</td><td class=text-center>{{machine.count}}</td></tr></tbody></table></div><div class=section ng-if=\"seasons.length > 0\"><h3>Seasons</h3><ul class=list-unstyled><li ng-repeat=\"season in seasons\">{{season.name}} <span ng-if=season.pivot.rookie>as a rookie</span><span ng-if=season.pivot.guest>as a guest</span><span ng-if=season.pivot.final_position>— Final position: {{season.pivot.final_position}}</span></li><li>&nbsp;</li><li ng-repeat=\"legacy in player.legacy_seasons.split(';').reverse()\">{{legacy}}</li></ul></div></div>"
  );


  $templateCache.put('public/views/results.html',
    "<ul class=list-unstyled><li ng-repeat=\"heat in heats\"><h2>{{season.name}}, {{heat.name}}</h2><ul class=list-unstyled><li ng-repeat=\"group in heat.groups | filter:hasGamesFilter\"><table class=\"table table-scores table-condensed\"><thead><tr><th>{{group.name}}</th><th ng-repeat=\"game in group.games\" class=text-center title={{game.machine.name}}>{{game.machine.shortname}}</th><th class=text-center><b>Total</b></th></tr></thead><tbody><tr ng-repeat=\"player in group.players\"><td><a href=#/player/{{player.player_id}}>{{player.display_name}}</a></td><td ng-repeat=\"game in group.games\" class=\"text-center {{ classForPlayerGame(player, game) }}\">{{ pointsForPlayer(game.results, player.player_id) }}</td><td class=text-center><b>{{ pointsForPlayer(group.points, player.player_id) }}</b></td></tr></tbody></table></li></ul></li></ul>"
  );


  $templateCache.put('public/views/standings.html',
    "<div class=\"landscape-tip alert alert-info\"><b>Tip:</b> Rotate your phone to landscape mode for a better view.</div><table class=\"table table-striped table-condensed\"><thead><tr><th>&nbsp;</th><th>Round:</th><th class=text-center ng-repeat=\"heat in season.heats\">#{{heat.delta+1}}</th><th ng-if=season.should_adjust_score class=text-center>Adj.</th><th class=text-center>Tot.</th></tr></thead><tbody><tr ng-repeat=\"seasonPoint in season.points | orderBy:(season.should_adjust_score ? 'adjusted_points' : 'points'):true\" class={{isFirstRookie(seasonPoint.player_id)}}><td class=index-cell>{{getPosition($index, (season.should_adjust_score ? seasonPoint.adjusted_points : seasonPoint.points))}}</td><td><a href=#/player/{{seasonPoint.player_id}} ng-bind-html=(nameForPlayer(seasonPoint.player_id))>{{ (nameForPlayer(seasonPoint.player_id)) }}</a></td><td class=text-center ng-repeat=\"heat in season.heats\">{{pointsForPlayerForHeat(heat.points, seasonPoint.player_id)}}</td><td ng-if=season.should_adjust_score class=text-center><b>{{seasonPoint.adjusted_points}}</b></td><td class=text-center><span ng-show=season.should_adjust_score>{{seasonPoint.points}}</span> <b ng-hide=season.should_adjust_score>{{seasonPoint.points}}</b></td></tr></tbody></table>"
  );
